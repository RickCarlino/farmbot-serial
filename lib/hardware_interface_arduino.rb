...
#print "#{i}"
  ...
  def check_emergency_stop
    if (HardwareInterface.current.status.emergency_stop)
     serial_port_write( "E\n" )
    end
  end
  ...
  def process_value_R81(params,code)
    if code == 'R81'
      HardwareInterface.current.status.info_end_stop_x_a = (params.xa == 1)
      HardwareInterface.current.status.info_end_stop_x_b = (params.xb == 1)
      HardwareInterface.current.status.info_end_stop_y_a = (params.ya == 1)
      HardwareInterface.current.status.info_end_stop_y_b = (params.yb == 1)
      HardwareInterface.current.status.info_end_stop_z_a = (params.za == 1)
      HardwareInterface.current.status.info_end_stop_z_b = (params.zb == 1)
    end
  end
  ...
  def process_value_R82(params,code)
    if code == 'R82'
      ...
      HardwareInterface.current.status.info_current_x_steps = params.x
      HardwareInterface.current.status.info_current_x       = params.x / @ramps_param.axis_x_steps_per_unit

      HardwareInterface.current.status.info_current_y_steps = params.y
      HardwareInterface.current.status.info_current_y       = params.y / @ramps_param.axis_y_steps_per_unit

      HardwareInterface.current.status.info_current_z_steps = params.z
      HardwareInterface.current.status.info_current_z       = params.z / @ramps_param.axis_z_steps_per_unit
  ...
  def process_value_process_R83(code,text)
    if code == 'R83'
        HardwareInterface.current.status.device_version = text
    end
  end
